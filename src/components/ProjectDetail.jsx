...
const FeatureItem = ({ feature }) => {
  return (
    <li className="group flex items-start gap-3 p-1.5 md:p-2 rounded-xl hover:bg-white/[0.04] transition-all duration-300 border border-transparent hover:border-white/10 leading-6">
      <div className="relative mt-1.5">
        <div className="absolute -inset-1 bg-gradient-to-r from-blue-600/20 to-purple-600/20 rounded-full blur group-hover:opacity-100 opacity-0 transition-opacity duration-300" />
        <div className="relative w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-gradient-to-r from-blue-400 to-purple-400 group-hover:scale-110 transition-transform duration-300" />
      </div>
...
const handleGithubClick = (githubLink) => {
  if (githubLink === 'Private') {
    Swal.fire({
      ...
      text: 'This repository is private due to university academic policy. I’m happy to walk through the code live or share snippets on request.',
          ...
          <div className="grid lg:grid-cols-2 gap-8 md:gap-16">
            <div className="space-y-6 md:space-y-10 animate-slideInLeft">
              ...
              <div className="flex flex-wrap gap-3 md:gap-4">
                {/* Action buttons */}
                {/* Show Live Demo only if a link is provided */}
            {project?.Link && (
              <a
                href={project.Link}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 rounded-xl px-5 py-3 bg-indigo-500/10 text-indigo-300 hover:bg-indigo-500/20 border border-indigo-400/20 transition"
              >
